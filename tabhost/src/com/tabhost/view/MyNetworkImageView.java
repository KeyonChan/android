...
import com.tabhost.MainActivity;
...
public class MyNetworkImageView extends ImageView {
	 ...
	 */
	public void setImageUrl(String url, ImageLoader imageLoader, int type, int reqWidth) {
		mUrl = url;
		mImageLoader = imageLoader;
		// The URL has potentially changed. See if we need to load it.
		loadImageIfNecessary(false, type, reqWidth);
	}
	...
	/**
	 * isInLayoutPass代表是否重新请求网络 ，true：重新请求 false：取缓存
	 ...
	 */ 
	private void loadImageIfNecessary(final boolean isInLayoutPass, final int type, final int reqWidth) {
		int width = getWidth();
			...
			public void onResponse(final ImageContainer response, boolean isImmediate) {
				...
				if (response.getBitmap() != null) {
					if(type == 0){
						...
						setImageBitmap(ImgUtil.getRoundBitmap(response.getBitmap()));
					else
						setImageBitmap(ImgUtil.getZoomBitmap(ApplicationDemo.context, response.getBitmap(), reqWidth));
